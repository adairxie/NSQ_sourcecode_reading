...
func (a *State) IsAllowed(topic, channel string) bool {
    for _, aa := ramge a.Authorizations {
...
func QueryAnyAuthd(authd []string, remoteIP, tlsEnable, authSecret string, connectTimeout time.Duration, requestTimeout time.Duration) (*State, error) {
    for _, a := range authd {
        authState, err := QueryAuthd(a, remoteIP, tlsEnable, authSecret, connectTimeout, requestTimeout)
        if err != nil {
...
func QueryAuthd(authd, remoteIP, tlsEnabled, authSecret string, connectTimeout time.Duration, requestTimeout time.Duration) (*State, error) {
    v := url.Values{}
