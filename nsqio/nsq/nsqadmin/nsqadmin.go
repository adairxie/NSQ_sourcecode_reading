...
import (
    ...
    "github.com/nsqio/internal/version"
)
...
func New(opts *Options) *NSQAdmin {
    ...
    if opts.HTTPClientTLSCert != "" && opts.HTTPClientTLSKey != "" {
        ...
        if err != nil {
            n.logf(LOG_FATAL, "failed to load LoadX509KeyPair %s, %s - %s", opts.HTTPClientTLSCert, opts.HTTPClientTLSKey, err)
            os.Exit(1)
        }
    ...
    if opts.HTTPClientTLSRootCAFile != "" {
        ...
        if err != nil {
            n.logf(LOG_FATAL, "failed to read TLS root CA file %s - %s", opts.HTTPClientTLSRootCAFile, err)
            os.Exit(1)
        }
        if !tlsCertPool.AppendCertsFromPEM(caCertFile) {
            n.logf(LOG_FATAL, "failed to AppendCertsFromPEM %s", opt.HTTPClientTLSRootCAFile)
            os.Exit(1)
        }
...
func (n *NSQAdmin) handleAdminActions() {
    for action := range n.notifications {
        ...
        resp, err := httpclient.Post(n.getOpts().NotificationHTTPEndpoint, "application/json", bytes.NewBuffer(content))
        if err != nil {
