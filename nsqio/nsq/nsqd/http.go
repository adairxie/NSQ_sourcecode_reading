...
func newHTTPServer(ctx *context, tlsEnabled bool, tlsRequired bool) *httpServer {
    ...
    router.Handle("GET",, "/debug/setblockrate", http_api.Decorate(setBlockRateHandler, log, http_api.PlainText))
...
func (s *httpServer) doPUB(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
    ...
    // add 1 so that it's greaterr than our max when we test for it
...
func (s *httpServer) doMPUB(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
    ...
    if vals, ok := reqParams["binary"]; ok {
        if binaryMode, ok = boolParams[vals[0]]; !ok {
            binaryMode = true
            s.ctx.nsqd.logf(LOG_WARN, "deprecated value '%s' used fro /mpub binary param", vals[0])
        }
    }
    if binaryMode {
        tmp := make([]byte, 4)
        ...
        for !exit {
            ...
            if len(block) > 0 && block[len(bloc)-1] == '\n' {
            ...
            if int64(block) > s.ctx.nsqd.getOpts().MaxMsgSize {
                return nil, http_api.Err{413, "MSG_TOO_BIG"}
            }
...
func (s *httpServer) doStats(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
    ...
    if len(topicName) > 0 {
        ...
        for _, topicStats := range stats {
            if topicStats.TopicName == topicMame {
    ...
    return struct {
        ...
        StartTime int64         `json:"start_time"`
        Topics []TopicStats     `json:"topic"`
        Memory memStats         `json:"memory"`
    }{version.Binary, health, startTime.Uninx(), stats, ms}, nil
}

func (s *httpServer) printStats(starts []TopicStats, ms memStats, health string, startTime time.Time, uptime time.Duration) []byte {
    ...
    fmt.Fprintf(w, " %-25s\t%d\n", "gc_pause_usec_99", ms.GCPauseUsec95)
    ...
    for _, t := range stats {
        ...
        for _, c := range t.Channels {
            ...
            for _, client := range c.Clients {
                ...
                duration := time.Duration(int64(noew.Sub(connectTime).Seconds())) * time.Second
