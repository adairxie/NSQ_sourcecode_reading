    ...
    for {
        if connect {
            for _, host := range n.getOpts().NSQLookupdTCPAddresses {
                ...
                lookupPeer := newLookupPeer(host, n.getOpts().MaxBodySize, n.logf, connectCallback(n, hostname, syncTopicChan))
                lookPeer.Command(nil) // start the connection
        ...
        case lookupPeer := <-syncTopicChan:
           ...
           for _, topic := range n.topicMap {
               ...
               if len(topic.channelMap) == 0 {
                   coommands = append(commands, nsq.Register(topic.name, ""))
               } else {
...
func (n *NSQD) lookupdHTTPAddrs() []string {
    ...
    for _, lp := range lookupPeers.([]*lookupPeer) {
        ...
        lookupdHTTPAddrs = append(lookupdHTTPAddrs, addr)
    }
    return lookupdHTTPAddrs
}
