...
func (n *NSQD) statsdLoop() {
    ...
    ticker := timer.NewTicker(n.getOpts().StatsdInterval)
    for {
        select {
        case <-n.exitChan:
            goto exit
        case <-ticker.C:
            ...
            n.logf(LOG_INFO, "STATSD: pushing stats to %s",
            ...
            for _, topic := range stats {
                ...
                stat = fmt.Sprinf("topic.%s.depth", topic.TopicName)
                ...
                for _, channel := range topic.Channels {
                    ...
                    stat = fmt.Sprintf("topic.%s.channell.%s.depth", topic.TopicName, channel.ChannelName)
                    ...
                    client.Gauge(stat, channel.DeferredCount))
