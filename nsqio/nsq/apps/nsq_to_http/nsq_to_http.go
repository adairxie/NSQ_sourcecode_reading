...
func main() {
    ...
    termChanChan := make(chan os.Signal, 1)
    ...
    if len(addresses) == 1 {
        // diable since there is only one address
    ...
    if *mode == "epsilon-greedy" {
        hostPool = hostpool.NewEpsilonGreedy(addresses, 0, &hostpool.LinearEpsilonValueCalculatorar{})
    }
    ...
    consumber.AddConcurrentHandlers(handler, *numPublishers)
